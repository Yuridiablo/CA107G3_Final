...
// clear_line		
		...
		if ("call_member".equals(action)) {
			...
			synchronized(wait_line) {
				...
				SendToMember.beCalled(vendor_no, tbl_size, mem_no, deadline, getServletContext());
		...
		if ("clear_line".equals(action)) {
			...
			synchronized(wait_line) {
				...
				SendToMember.beCanceled(vendor_no, tbl_size, memList, getServletContext());
			}
		...
		if ("check_member".equals(action)) {
			...
			synchronized(wait_line) {
				...
				SendToMember.renewGpBeforeCheck("check", wait_line, tbl_size, vendor_no, getServletContext());
				SendToMember.beChecked(vendor_no, tbl_size, mem_no, getServletContext());
			}
		...
		if ("cancel".equals(action)) {
			...
			synchronized(wait_line) {
				...
				if (personInLine != null) {
					...
					SendToMember.renewGpBeforeCancel("cancel", wait_line, tbl_size, vendor_no, pilIdx, getServletContext());
				} else {
