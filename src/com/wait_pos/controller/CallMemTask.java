    	...
    	synchronized(wait_line) {
    		...
    		if (pil.getHasPassed()) { // 過號過, 直接移除
    			...
    			if (wait_line_queue.size() >= 4) { // 隊伍有 4 人以上, 往後移 3 位
        			...
        			SendToMember.renewGpBeforeCall("putBack", wait_line, tbl_size, vendor_no, 3, context);
    				...
    				SendToMember.renewGpBeforeCall("putBack", wait_line, tbl_size, vendor_no, wait_line_queue.size() - 1, context);
    			}
